...
class ClipRaster:
    ...
    def mask_shp(self, shp_path: str, crs=None, exact = False, scale_factor=10):
        ...
        if crs:
            ...
            for geom in shp.geometry:
                ...
                verts = geom.exterior.coords.xy
                verts = np.column_stack((verts[0],verts[1]))
                tupVerts_.append(verts)
        ...
        for tupVerts in tupVerts_[:1]:
            ...
            if not exact:
                ...
                weights = np.zeros(self.shape) * np.nan
            ...
            LANDMASK.append(mask)


        return WEIGHTS, LANDMASK
